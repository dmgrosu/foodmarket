...
| 8b | the ERP `products-data.xml` export | Data-side, not code. Every `<group>` mirrors a product one-for-one (same `code`, same `name`, `parentCode` = the product's `groupCode`), and the 1,231 codes products are actually filed under are never declared with a name. *(Worked around: `ProductGroupNaming` derives a name from the words a group's products all begin with — "ПОСУДА ЧАШКА", "НОСКИ мужские" — which covers all 1,231 with no two groups colliding. It is inference; a name the export declares wins over it. The real fix is one `<group code name>` line per category in the export.)* |
